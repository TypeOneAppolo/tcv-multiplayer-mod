...
GAME_VERSION = "0.5.1"
## The exact official Windows build this mod was made against. A different size
## does not necessarily mean it will not work, but it does mean the patches were
## never tested against whatever you have.
KNOWN_GAME_SIZE = 211382192
...
def check_game_exe(exe: Path, force: bool) -> None:
    if not exe.is_file():
        raise Failed(f"no such file: {exe}")
    size = exe.stat().st_size
    if size == KNOWN_GAME_SIZE:
        return
    msg = (f"{exe.name} is {size} bytes; the tested {GAME_VERSION} build is "
           f"{KNOWN_GAME_SIZE}.\n"
           "  This is probably a different version or platform of the game.")
    if not force:
...
def ensure_templates(cache: Path) -> None:
    dest = templates_dir()
    if (dest / "windows_release_x86_64.exe").is_file():
        say("skip", f"export templates already installed in {dest}")
        return
    archive = download(TEMPLATES_URL, cache / f"godot-templates-{GODOT_VERSION}.tpz")
    say("work", f"installing export templates to {dest}")
    dest.mkdir(parents=True, exist_ok=True)
    with zipfile.ZipFile(archive) as zf:
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--force", action="store_true",
                    help="continue even if the game exe is an untested version")
    ...
    if not args.game_exe:
        ap.error("give me the path to your game exe, or use --project")

    exe = Path(args.game_exe)
    check_game_exe(exe, args.force)
